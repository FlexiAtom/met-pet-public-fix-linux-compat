...
class MeaPet(QWidget):
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        safe_print(f"[settings] 加载配置: "
           ...
           f"reply={self._settings['bubble_duration_ms']['reply']}, "
           f"watch={self._settings['bubble_duration_ms']['watch']}, "
           f"interaction={self._settings['bubble_duration_ms']['interaction']}, "
           f"sync_with_audio={self._settings['tts']['sync_with_audio']}")
    ...
    def _load_settings(self) -> dict:
        """加载 config_settings.json，不存在则返回默认值"""
        default = {
            "bubble_duration_ms": {
                "default": 5000,
                "reply": 8000,
        ...
        try:
            ...
            safe_print(f"[settings] 加载失败，使用默认值: {e}")
            return default
    ...
    def _on_head_patted(self):
        ...
        self._interaction_speak(text, self._settings["bubble_duration_ms"]["interaction"], mood)
    ...
    def _show_bubble(self, text: str, duration_ms: int = None):
        if duration_ms is None:
            duration_ms = self._settings["bubble_duration_ms"]["default"]
    ...
    def show_reply(self, text: str, mood: str = "neutral", duration_ms: int = None):
        if duration_ms is None:
            duration_ms = self._settings["bubble_duration_ms"]["reply"]
        self._safe_set_mood(mood)
    ...
    def _do_chat(self, message: str):
        ...
        # 显示思考中提示
        self._show_bubble("💭 梅尔正在思考……", self._settings["bubble_duration_ms"]["thinking"])  # 0 = 持久显示
        self._position_bubble()
    ...
    def _on_watch_result(self, text: str, mood: str):
        ...
        try:
            ...
            self.show_reply(text, mood, duration_ms=self._settings["bubble_duration_ms"]["watch"])
            self._awaiting_reply = False
            self._start_watcher_timer()
    ...
    def _on_watch_tts_and_show(self, raw: str, reply: str = None, mood: str = None):
        ...
        if raw is None or reply is None:
            ...
            if reply and mood:
                self.show_reply(reply, mood, duration_ms=self._settings["bubble_duration_ms"]["watch"])
    ...
    def _on_watch_error(self, err: str):
        ...
        # 显示简短提示，不打扰主人
        self._awaiting_reply = False
        self._show_bubble(f"唔…看不清喵 ({err[:30]})", self._settings["bubble_duration_ms"]["default"])
    ...
    def _on_watch_silent(self):
        """视觉模型评估后决定不说话——安静恢复"""
        self._awaiting_reply = False
        self._show_bubble("😼 没什么好说的喵…", self._settings["bubble_duration_ms"]["default"])
